...
import { Download, File, Loader2, Trash2, Calendar, HardDrive } from 'lucide-react';
...
export const FileList: React.FC = () => {
  ...
  const downloadFile = async (file: FileRecord) => {
    try {
      setDownloadStatus({ fileId: file.id, status: 'downloading', progress: 30 });
      ...
      setDownloadStatus({ fileId: file.id, status: 'decrypting', progress: 60 });
      ...
      setDownloadStatus({ fileId: file.id, status: 'decrypting', progress: 80 });

      // Decrypt the file
      const decryptedData = await decryptFile(encryptedArrayBuffer, decryptionKey, iv);
  ...
  return (
    <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
      <CardHeader>
        ...
        <CardDescription>
          Your encrypted files. Click download to decrypt and save to your device.
        </CardDescription>
      </CardHeader>
                ...
                {downloadStatus?.fileId === file.id ? (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-muted-foreground">
                      {downloadStatus.status === 'downloading' ? 'Downloading...' : 'Decrypting...'}
                    </span>
                  </div>
                ) : (
                  <>
